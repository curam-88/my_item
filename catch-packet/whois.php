...
{
 $domain = trim($domain); //去掉开头结尾
 if(substr(strtolower($domain), 0, 7) == "http://") $domain = substr($domain, 7); //去掉http
 if(substr(strtolower($domain), 0, 4) == "www.") $domain = substr($domain, 4);//去掉www
...
//开始连接数据库
$con = mysql_connect("localhost","root","root");//域名(默认:locahost)、用户名、密码
if (!$con)
  {
  die('Could not connect: ' . mysql_error());//错误终止代码，退出
  }
//使用 mysql_query() 函数向 MySQL 发送查询或命令
//mysql_select_db("chenyuzhe", $con);//选择数据库
mysql_query("set names utf8");//数据库编码
//select 域名
$num=mysql_query("SELECT num FROM packet");
$sel=mysql_query("SELECT Host FROM packet");
get_host($sel,$num);
function get_host($sel,$num){
//查询
$whois=new Whois;
$query=$whois->whoislookup($server,$url);
//$whois=new Whois;
//$query=$whois->whoislookup("www.wanwang.com");

//处理数组,拆分提取单位和电话

$Array=array();
//var_dump(variable_to_string($whois));
$Array=(preg_split("/:|\n/",$query));
echo $company=$Array[22];
echo $phone=$Array[28];


//插入数据库
$sql = "insert into  whois value($num,'$company','$phone')";
mysql_query($sql,$con);
}
